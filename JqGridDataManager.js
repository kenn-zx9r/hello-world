 ...
 * =================================================================================================
*/
function JqGridDataManager(settings){
	 ...
	 * Static変数
	 */
	arguments.callee.jqGridResponse = {
		userdata: {
			result: true,
			errorCode: 0,
			error: ""
//			records: 0
	...
	arguments.callee.initStatus = {
		lastSelect: {
			request:		null,
			countSql:		null,
			countParams:	null,
			selectSql:		null,	//offset ... を除いたselect文が格納される
			selectPagerSql:	null,	//offset ... の句が格納される
			selectParams:	null,	//bind変数の配列が格納される
			response:		null
		},
		colModel: null
	};
	...
	arguments.callee.ENABLE_OFFSET_DBNAMES = [
		"PostgreSQL"
	];
	...
	this.settings.enableOffset = (arguments.callee.ENABLE_OFFSET_DBNAMES.indexOf(this.settings.db.databaseName) != -1);
	...
	this.status = $.extend({}, arguments.callee.initStatus);
	
	//カスタムメソッドの登録
	if(settings.customMethods){
...
JqGridDataManager.prototype = {
	 ...
	 * =============================================================================================
	 */
	select: function(request, options){
		var nowSelect = {};
